# Admin Front (Admin Panel) — CEC Electronic

This document explains how the **admin panel** of CEC Electronic works: admin login, the dashboard, and managing products, categories, brands, suppliers, orders, customers and delivery zones, plus Bakong payment checking and receipts. The full source code of every admin file is included at the end.

> For product details see `product.md`. For the customer side see `userfront.md`.

---

## 1. Overview

| Menu (sidebar) | URL | Controller | What the admin can do |
|----------------|-----|------------|------------------------|
| Dashboard | `/admin` | `Admin/ProductController@dashboard` | See stats, payment notifications, latest orders and latest products |
| Products | `/admin/products` | `Admin/ProductController` | List (filter by brand), create, edit, delete, upload images |
| Categories | `/admin/categories` | `Admin/CategoryController` | Create, edit, delete; set a parent category, image, sort order and active flag |
| Brands | `/admin/brands` | `Admin/BrandController` | Create, edit, delete; upload a logo, set sort order and active flag |
| Suppliers | `/admin/suppliers` | `Admin/SupplierController` | Create, edit, delete supplier contact information |
| Orders | `/admin/orders` | `Admin/OrderController` | List, view details, update status and delivery, verify Bakong payment, download receipt |
| Customers | `/admin/customers` | `Admin/CustomerController` | Customers grouped by phone number, with order history and total spent |
| Delivery | `/admin/delivery-zones` | `Admin/DeliveryZoneController` | Create, edit, delete delivery zones (fee, free-delivery minimum, estimated days) |

All pages use the shared layout `resources/views/admin/layout.blade.php`: a sidebar menu, a top bar with a logout button, flash messages, and the loading overlay.

---

## 2. Admin login and security

### How an admin account is created
- The `users` table has an `is_admin` column (boolean, default `false`), added by the `add_is_admin_to_users_table` migration.
- `DatabaseSeeder` creates or updates the admin user from `.env`:
  ```env
  ADMIN_EMAIL=admin@cecelectronic.com
...
### Login flow
```
GET  /admin/login   → admin/auth/login.blade.php
POST /admin/login   → Admin\AuthController@store
        • validates email + password
        • Auth::attempt(...) must succeed AND user->is_admin must be true
          (if not, the user is logged out and "The admin email or password is incorrect." is shown)
        • session()->regenerate()
        • session('admin_authenticated') = true
        → redirect to /admin (or the page they were trying to reach)
POST /admin/logout  → removes admin_authenticated, logs out, clears the session → /admin/login
```

### Middleware: `EnsureAdminSession`
Every `/admin/*` route (except login and logout) passes through this middleware. It checks three things:
1. The user is logged in (`Auth::check()`)
2. `user->is_admin` is true
3. The session has `admin_authenticated = true`

If any check fails, it redirects to `/admin/login`. A normal customer who is logged in on the shop **cannot** open the admin panel.

> Login and logout POST routes skip CSRF checking (`withoutMiddleware(PreventRequestForgery)`), and the login form can also submit by AJAX and receive JSON.
...
Route::prefix('admin')->name('admin.')->middleware(EnsureAdminSession::class)->group(function () {
    ...
    Route::get('orders/{order}/receipt', ...)->name('orders.receipt');
    Route::post('orders/{order}/verify-payment', ...)->name('orders.verify-payment');
    Route::get('customers', ...)->name('customers.index');
    Route::get('customers/{phone}', ...)->name('customers.show');
...
### 4.1 Dashboard (`/admin`)
Shows these stats:
- Total products, categories and orders
- Customers (distinct `customer_phone` values in orders)
- Revenue (`SUM(grand_total)` of all orders)
- **Payment notifications**: orders where `payment_confirmed_at` is set but `admin_payment_seen_at` is empty (new paid orders the admin hasn't opened yet)
- **Low stock**: products with `stock_quantity <= 5`
- Total of all product prices

It also lists the 6 latest products, the 6 latest orders, and the 6 latest unseen payment notifications.

### 4.2 Products
- List: 10 per page, loads category, brand and supplier, with a `?brand=` filter.
- Create and edit share `_form.blade.php`. Validation: name and price are required; SKU must be unique; the image can be jpg, png, webp, svg and similar formats, up to 4 MB.
- The slug is generated from the name (`-2`, `-3` and so on are added if it's already taken). When an image is replaced, the old file is deleted.
- Saving or deleting a product clears the cache, so the shop updates right away.
- Details: see `product.md`.

### 4.3 Categories
- Fields: `parent_id` (optional parent category), `name`, `description`, `image`, `is_active`, `sort_order`.
- The slug is generated automatically and made unique. Images are stored on the public disk under `categories/`.
- The edit form doesn't list the category itself as a possible parent.

### 4.4 Brands
- Fields: `name`, `logo` (jpg, png or webp, up to 4 MB, stored under `brands/`), `is_active`, `sort_order`.
- The list shows how many products each brand has.
- Deleting a brand **keeps its products**: `brand_id` is set to null.
- Only brands that are active **and have a logo** appear on the shop home page.

### 4.5 Suppliers
- Fields: name, company, email, phone, website, address, contact person, payment terms, active flag, notes.
- The list shows how many products each supplier has. Only active suppliers can be chosen in the product form.

### 4.6 Orders
- **List:** 10 per page, with delivery provider, delivery zone and item count, plus a count of unseen payment notifications.
- **Details (`show`):** opening a paid order that the admin hasn't seen yet sets `admin_payment_seen_at = now()`, which clears the notification.
- **Update:** the admin changes
  - `status` (pending, processing, shipped, delivered, and so on)
  - `payment_status` (changing it to `paid` sets `payment_confirmed_at` and `admin_payment_seen_at`)
  - delivery zone, delivery provider, tracking number, shipped date and delivered date.
  If a provider or tracking number is set, a **Shipment** is created or updated. Its status is `delivered`, `shipped` or `pending`, depending on which dates are filled in.
- **Verify payment:** for Bakong orders, the admin can press a button to check the payment with Bakong right away. This skips the daily API limit and has a 10-second guard against double clicks. If the payment is found, the order is marked paid.
- **Receipt:** downloads a PDF using `ReceiptPdf`.

### 4.7 Customers
- There is no separate customers table. Customers are built by **grouping orders by `customer_phone`**: name, email, number of orders, total spent and last order date.
- The detail page (`/admin/customers/{phone}`) lists all orders for that phone number, with stats for orders, spent, unpaid and latest order.

### 4.8 Delivery zones
- Fields: name, city, province, `delivery_fee`, `free_delivery_minimum`, `estimated_days` (at least 1), active flag.
- Zones and providers can be assigned to an order on the order detail page.

### 4.9 Background payment check
`app/Console/Commands/CheckPendingBakongPayments.php` (`bakong:check-pending`) runs **every 5 minutes** (set in `routes/console.php`). It checks up to 5 unpaid Bakong orders from the last 2 hours, newest first, and marks them paid when Bakong confirms the payment. This means a paid order is still confirmed even if the customer closed the browser tab. You need to run `php artisan schedule:work` (or a cron job) for it to run.
...
```
/admin/login ──(is_admin + password OK)──► /admin (Dashboard)
                                              │
     ┌──────────┬──────────┬──────────┬───────┼──────────┬───────────┬──────────┐
  Products  Categories  Brands   Suppliers  Orders   Customers   Delivery   Logout
  (CRUD)     (CRUD)     (CRUD)    (CRUD)      │       (read)      (CRUD)
                                              ▼
                                  Order detail → update status/delivery
...
### 6.3 `app/Http/Middleware/EnsureAdminSession.php`
...
### 6.4 `app/Http/Controllers/Admin/AuthController.php`
...
### 6.5 `app/Http/Controllers/Admin/ProductController.php`
...
### 6.6 `app/Http/Controllers/Admin/CategoryController.php`
...
### 6.7 `app/Http/Controllers/Admin/BrandController.php`
...
### 6.8 `app/Http/Controllers/Admin/SupplierController.php`
...
### 6.9 `app/Http/Controllers/Admin/OrderController.php`
...
### 6.10 `app/Http/Controllers/Admin/CustomerController.php`
...
### 6.11 `app/Http/Controllers/Admin/DeliveryZoneController.php`
...
### 6.12 `app/Console/Commands/CheckPendingBakongPayments.php`
...
### 6.13 `app/Services/ReceiptPdf.php`
...
### 6.14 `database/seeders/DatabaseSeeder.php`
...
### 6.15 `database/migrations/2026_05_20_000002_add_is_admin_to_users_table.php`
...
### 6.17 `app/Models/DeliveryZone.php`
...
### 6.18 `app/Models/DeliveryProvider.php`
...
### 6.20 `resources/views/admin/layout.blade.php`
...
### 6.21 `resources/views/admin/auth/login.blade.php`
...
### 6.22 `resources/views/admin/dashboard.blade.php`
...
### 6.23 `resources/views/admin/products/index.blade.php`
...
### 6.24 `resources/views/admin/products/create.blade.php`
...
### 6.25 `resources/views/admin/products/edit.blade.php`
...
### 6.26 `resources/views/admin/products/_form.blade.php`
...
### 6.27 `resources/views/admin/categories/index.blade.php`
...
### 6.28 `resources/views/admin/categories/create.blade.php`
...
### 6.29 `resources/views/admin/categories/edit.blade.php`
...
### 6.30 `resources/views/admin/categories/_form.blade.php`
...
### 6.31 `resources/views/admin/brands/index.blade.php`
...
### 6.32 `resources/views/admin/brands/create.blade.php`
...
### 6.33 `resources/views/admin/brands/edit.blade.php`
...
### 6.34 `resources/views/admin/brands/_form.blade.php`
...
### 6.35 `resources/views/admin/suppliers/index.blade.php`
...
### 6.36 `resources/views/admin/suppliers/create.blade.php`
...
### 6.37 `resources/views/admin/suppliers/edit.blade.php`
...
### 6.38 `resources/views/admin/suppliers/_form.blade.php`
...
### 6.39 `resources/views/admin/delivery-zones/index.blade.php`
...
### 6.40 `resources/views/admin/delivery-zones/create.blade.php`
...
### 6.41 `resources/views/admin/delivery-zones/edit.blade.php`
...
### 6.42 `resources/views/admin/delivery-zones/_form.blade.php`
...
### 6.43 `resources/views/admin/orders/index.blade.php`
...
### 6.44 `resources/views/admin/orders/show.blade.php`
...
### 6.45 `resources/views/admin/customers/index.blade.php`
...
### 6.46 `resources/views/admin/customers/show.blade.php`
